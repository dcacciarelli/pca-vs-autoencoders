...
class OrthogonalAutoencoder(nn.Module):
    def __init__(self, input_size, encoding_size, orthogonality_weight):
    ...
    def custom_loss(self, compression, reconstruction, original):
        loss = torch.norm((reconstruction - original), p=2) + \
               self.orthogonality_weight * torch.norm((torch.matmul(compression.T, compression) - torch.eye(compression.shape[1])), p=2)
        return loss

# Train the sparse autoencoder to reconstruct the input data with sparsity regularization
sparse_autoencoder = OrthogonalAutoencoder(input_size=inp_shape, encoding_size=5, orthogonality_weight=1)
